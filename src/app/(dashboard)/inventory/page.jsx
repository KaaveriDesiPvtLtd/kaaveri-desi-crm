...
const Inventory = () => {
  ...
  const formatStockDisplay = (stock, unit) => {
    if (!stock) return { value: 0, unit: unit || 'units' };
    const u = (unit || '').toLowerCase().trim();
    if (u === 'gm') {
      return { 
        value: stock >= 1000 ? parseFloat((stock / 1000).toFixed(3)) : stock, 
        unit: stock >= 1000 ? 'kg' : 'gm' 
      };
    }
    if (u === 'ml') {
      return { 
        value: stock >= 1000 ? parseFloat((stock / 1000).toFixed(3)) : stock, 
        unit: stock >= 1000 ? 'ltr' : 'ml' 
      };
    }
    return { value: stock, unit: u || 'units' };
  };
                      ...
                      >
                        ...
                        <td className="px-4 py-4 text-center">
                          <span className={`font-bold text-sm ${product.currentStock <= 5 ? 'text-red-600 bg-red-50 px-2 py-1 rounded-md' : 'text-slate-700'}`}>
                            {formatStockDisplay(product.currentStock, product.unit).value}
                            <span className="ml-1 text-[10px] text-slate-400 font-normal uppercase">{formatStockDisplay(product.currentStock, product.unit).unit}</span>
                          </span>
                        </td>
                        {canStock && (
                          <td className="px-4 py-4 text-center">
                          ...
                          <td colSpan={colCount} className="px-6 py-4">
                            <div className="ml-6 space-y-3">
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <div className="text-xs font-medium text-slate-400">Total Stock: <span className="text-slate-700 font-bold">{formatStockDisplay(product.currentStock, product.unit).value} {formatStockDisplay(product.currentStock, product.unit).unit.toUpperCase()}</span></div>
                              </div>
